...
class ModulesModelModules extends JModelList
{
	...
	protected function populateState($ordering = null, $direction = null)
	{
		...
		$clientId = $this->getUserStateFromRequest($this->context.'.filter.client_id', 'filter_client_id', 1, 'int', false);
		$previousId = $app->getUserState($this->context.'.filter.client_id_previous', null);
		if($previousId != $clientId || $previousId === null){
			$this->getUserStateFromRequest($this->context.'.filter.client_id_previous', 'filter_client_id_previous', 1, 'int', true);
	...
	protected function translate(&$items)
	{
		...
		$client = JApplicationHelper::getClientInfo($filter_client);
		foreach($items as $item) {
			$extension = $item->module;
			$source = $client->path . "/modules/$extension";
				$lang->load("$extension.sys", $client->path, null, false, false)
			||	$lang->load("$extension.sys", $source, null, false, false)
			||	$lang->load("$extension.sys", $client->path, $lang->getDefault(), false, false)
